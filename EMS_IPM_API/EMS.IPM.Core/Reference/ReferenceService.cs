...
using EMS.IPM.Transfer.Reference;
...
namespace EMS.IPM.Core.Reference
{
    public interface IReferenceService
    {
        ...
        Task<IActionResult> UpdateSet(APICredentials credentials,
            List<Utilities.API.ReferenceMaintenance.ReferenceValue> param);

        Task<IActionResult> GetIDByAutoComplete(APICredentials credentials, GetAutoCompleteInput param);

        Task<IActionResult> GetDropDown(APICredentials credentials, List<string> RefCodes, string ID);
        ...
        Task<IActionResult> GetByRefCode(APICredentials credentials, string RefCode);
    }
    ...
    public class ReferenceService : Core.Shared.Utilities, IReferenceService
    {
        ...
        public ReferenceService(IPMContext dbContext, IConfiguration iconfiguration,
        ...
        public async Task<IActionResult> GetByRefCodes(APICredentials credentials, List<string> RefCodes)
        {
            ...
            //return new OkObjectResult(SharedUtilities.GetDropdown((await _dbAccess.GetByRefCodes(RefCodes)).ToList(), "Value", "Description", "Description", null));
        ...
        public async Task<IActionResult> GetDropDown(APICredentials credentials, List<string> RefCodes, string ID)
        {
            return new OkObjectResult(SharedUtilities.GetDropdown((await _dbAccess.GetByRefCodes(RefCodes)).ToList(), "Value", "Description", "", ID));
            //return new OkObjectResult(
        ...
        public async Task<IActionResult> GetReferenceValueList(APICredentials credentials, string RefCode)
        {
            return new OkObjectResult(
                ...
                .Select(x => new GetReferenceValueListOutput
                {
                    ID = x.ID,
                    Value = x.Value,
                    Description = x.Description
        ...
        public async Task<IActionResult> AddReferenceValue(APICredentials credentials, Utilities.API.ReferenceMaintenance.ReferenceValue param)
        {
            ...
            if ((await _dbAccess.GetByRefCodeValue(param.RefCode, param.Value)).Count() > 0)
            {
        ...
        public async Task<IActionResult> UpdateReferenceValue(APICredentials credentials, Utilities.API.ReferenceMaintenance.ReferenceValue param)
        {
            ...
            if ((await _dbAccess.GetByRefCodeValue(param.RefCode, param.Value)).Where(x => x.ID != param.ID).Count() > 0)
            {
                ErrorMessages.Add("Reference Value " + MessageUtilities.SUFF_ERRMSG_REC_EXISTS);
            }
        ...
        {
            return new OkObjectResult(await _dbAccess.GetValueByRefCodeValue(param.RefCode, param.Value));
        }

        public async Task<IActionResult> GetByRefCode(APICredentials credentials, string RefCode)
        {
            return new OkObjectResult(await _dbAccess.GetValueByRefCode(RefCode));
        }
