...
namespace EMS.Recruitment.Core.Reference
{
    public interface IReferenceService
    {
        ...
        Task<IActionResult> Post(APICredentials credentials, List<Utilities.API.ReferenceMaintenance.ReferenceValue> param);
    ...
    public class ReferenceService : Core.Shared.Utilities, IReferenceService
    {
        ...
        public ReferenceService(RecruitmentContext dbContext, IConfiguration iconfiguration,
        ...
        {
            if (RefCodes.Contains("LEGAL_PROFILE"))
            {
                return new OkObjectResult(
        ...
        public async Task<IActionResult> Post(APICredentials credentials, List<Utilities.API.ReferenceMaintenance.ReferenceValue> param)
        {
            ...
            foreach (var item in param)
            {
                ...
                if ((await _dbAccess.GetByRefCodeValue(item.RefCode, item.Value)).Count() > 0)
                {
            ...
            {
                await _dbAccess.Post(param.Select(x => new Data.Reference.ReferenceValue
                {
                    RefCode = x.RefCode,
                    Value = x.Value,
                    Description = x.Description,
