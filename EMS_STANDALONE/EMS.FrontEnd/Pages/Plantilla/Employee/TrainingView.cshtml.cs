...
{
    public class TrainingViewModel : SharedClasses.Utilities
    {
        [BindProperty]
        public EMS.Workflow.Transfer.Training.EmployeeTrainingForm EmployeeTraining { get; set; }

        public TrainingViewModel(IConfiguration iconfiguration, IWebHostEnvironment env) : base(iconfiguration, env)
        { }
        public virtual async Task OnGetAsync(int ID)
        {
            ViewData["Function"] = "View";
            ViewData["HasEdit"] = _systemURL.Where(x => x.URL.ToUpper().Equals("/PLANTILLA/EMPLOYEE/TRAININGEDIT")).Count() > 0 ? "true" : "false";
