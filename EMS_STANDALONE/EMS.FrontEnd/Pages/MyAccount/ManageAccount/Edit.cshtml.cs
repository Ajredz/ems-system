...
namespace EMS.FrontEnd.Pages.MyAccount.ManageAccount
{
    public class EditModel : SharedClasses.Utilities
    {
        ...
        [BindProperty]
        public EMS.Plantilla.Transfer.Employee.UpdateProfileForm UpdateProfileForm { get; set; }
        ...
        {
            UpdateProfileForm.CreatedBy = _globalCurrentUser.UserID;

            var URL = string.Concat(_plantillaBaseURL,
                               _iconfiguration.GetSection("PlantillaService_API_URL").GetSection("Employee").GetSection("PostUpdateProfile").Value, "?",
                               "userid=", _globalCurrentUser.UserID);

            var (IsSuccess, Message) = await SharedUtilities.PostFromAPI(UpdateProfileForm, URL);
            ...
            if (IsSuccess)
            {
                ...
                await new Common_AuditLog(_iconfiguration, _globalCurrentUser, _env)
                    .AddAuditLog(new Security.Transfer.AuditLog.Form
                    {
                        ...
                        TableName = "Request Update Employee Profile",
                        TableID = 0, // New Record, no ID yet
                        Remarks = string.Concat(Employee.Code, " request update profile"),
                        IsSuccess = true,
        ...
        public async Task<JsonResult> OnGetFieldsDropDown()
        {
            var res = await new Common_Reference(_iconfiguration, _globalCurrentUser, _plantillaBaseURL, _env)
                .GetReferenceValueByRefCode(EMS.Plantilla.Transfer.Enums.ReferenceCodes.UPDATEPROFILE.ToString());

            _resultView.Result =
                (JsonConvert.DeserializeObject<List<SelectListItem>>(res.Select(x => x.Description).FirstOrDefault())).Select(
                    x => new SelectListItem
                    {
                        Value = x.Value,
                        Text = x.Text
        ...
        public async Task<JsonResult> OnGetUpdateProfileTags()
        {
            var res = await new Common_Reference(_iconfiguration, _globalCurrentUser, _plantillaBaseURL, _env)
                .GetReferenceValueByRefCode(EMS.Plantilla.Transfer.Enums.ReferenceCodes.UPDATEPROFILETAGS.ToString());
        ...
        public async Task<JsonResult> OnGetFieldsValue()
        {
