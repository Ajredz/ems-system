...
namespace EMS.FrontEnd.Pages.LogActivity.AssignedActivities
{
    public class IndexModel : SharedClasses.Utilities
    {
        ...
        public void OnGet()
        {
            if (_systemURL != null)
            {
                ViewData["HasEditFeature"] = _systemURL.Where(x => x.URL.ToUpper().Equals("/LOGACTIVITY/ASSIGNEDACTIVITIES/BATCHUPDATE")).Count() > 0 ? "true" : "false";
            }
        }

        public async Task<IActionResult> OnGetList([FromQuery] GetAssignedActivitiesListInput param)
        {
            ...
            // Get Applicant Names by Applicant IDs
            List<EMS.Recruitment.Transfer.Applicant.Form> applicants =
               ...
               .GetApplicants(Result.Where(x => x.ApplicantID != 0).Select(x => x.ApplicantID)
               .Distinct().ToList()));

            // Get OrgGroup by OrgGroup IDs
            ...
            if (systemUsers.Count > 0)
            {
                ...
                Result = Result
                       ...
                       (x, y) => new { activities = x, employees = y })
                       .Select(x => new GetAssignedActivitiesListOutput
                       {
                           ...
                           ApplicantID = x.activities.activities.ApplicantID,
            ...
            if (employees.Count > 0)
            {
                ...
                Result = Result
                       ...
                       .Select(x => new GetAssignedActivitiesListOutput
                       {
                           ...
                           CurrentStatusCode = x.activities.activities.CurrentStatusCode,
                           Remarks = x.activities.activities.Remarks,
                           EmployeeName = x.employees == null ? "" : x.employees.PersonalInformation == null ? "" :
                               string.Concat((x.employees.PersonalInformation.LastName ?? "").Trim(),
                                    string.IsNullOrEmpty((x.employees.PersonalInformation.FirstName ?? "").Trim()) ? ""
                                        : string.Concat(", ", x.employees.PersonalInformation.FirstName),
                                    string.IsNullOrEmpty((x.employees.PersonalInformation.MiddleName ?? "").Trim()) ? ""
            ...
            if (applicants.Count > 0)
            {
                ...
                Result = Result
                       ...
                       (x, y) => new { activities = x, applicants = y })
                       ...
                       .Select(x => new GetAssignedActivitiesListOutput
                       {
                           ...
                           CurrentStatusCode = x.activities.activities.CurrentStatusCode,
                           Remarks = x.activities.activities.Remarks,
                           EmployeeName = x.activities.activities.EmployeeName,
                           ApplicantName = x.applicants == null ? "" : x.applicants.PersonalInformation == null ? "" :
                               string.Concat((x.applicants.PersonalInformation.LastName ?? "").Trim(),
                                    string.IsNullOrEmpty((x.applicants.PersonalInformation.FirstName ?? "").Trim()) ? ""
                                        : string.Concat(", ", x.applicants.PersonalInformation.FirstName),
                                    string.IsNullOrEmpty((x.applicants.PersonalInformation.MiddleName ?? "").Trim()) ? ""
                                        : string.Concat(" ", x.applicants.PersonalInformation.MiddleName))
                       }).ToList();
            }
            ...
            if (orgGroup.Count > 0)
            {
                ...
                Result = Result
                       ...
                       .Select(x => new GetAssignedActivitiesListOutput
                       {
                           ...
                           Remarks = x.activities.activities.Remarks,
                           EmployeeName = x.activities.activities.EmployeeName,
                           ApplicantName = x.activities.activities.ApplicantName,
                           OrgGroupName = x.orgGroup == null ? "" :
                               string.Concat(x.orgGroup.Code, " - ", (x.orgGroup.Description ?? "").Trim())
                       }).ToList();
            }
        ...
        public async Task<(List<GetAssignedActivitiesListOutput>, bool, string)> GetExportData([FromQuery] GetAssignedActivitiesListInput param, bool IsExport)
        {
            var URL = string.Concat(_workflowBaseURL,
                  _iconfiguration.GetSection("WorkflowService_API_URL").GetSection("LogActivity").GetSection("GetAssignedActivitiesList").Value, "?",
                  ...
                  "ApplicantDelimited=", param.ApplicantDelimited, "&",
                  ...
                  "CurrentTimstampFrom=", param.CurrentTimstampFrom, "&",
                  "CurrentTimstampTo=", param.CurrentTimstampTo, "&",
            ...
            return await SharedUtilities.GetFromAPI(new List<GetAssignedActivitiesListOutput>(), URL);
        ...
        public async Task<JsonResult> OnGetEmployeeAutoComplete(string Term, int TopResults)
        {
        ...
        public async Task<IActionResult> OnGetCheckExportList([FromQuery] GetAssignedActivitiesListInput param)
        {
        ...
        public async Task<IActionResult> OnGetDownloadExportList([FromQuery] GetAssignedActivitiesListInput param)
        {
            var (Result, IsSuccess, ErrorMessage) = await GetExportData(param, true);
            ...
            if (IsSuccess)
            {
                ...
                string sFileName = _IsAdminAccess == true ? "All Tasks.xlsx" : "Assigned Tasks.xlsx";
                var memory = new MemoryStream();
                using (var fs = new FileStream(Path.Combine(sWebRootFolder, sFileName), FileMode.Create, FileAccess.Write))
                {
                    IWorkbook workbook;
                    workbook = new XSSFWorkbook();
                    ISheet excelSheet = workbook.CreateSheet(_IsAdminAccess == true ? "All Tasks" : "Assigned Tasks");
                    int irow = 0;
                    ...
                    row.CreateCell(6).SetCellValue("Current Status");
                    row.CreateCell(7).SetCellValue("Employee Name");
                    row.CreateCell(8).SetCellValue("Applicant Name");
                    row.CreateCell(9).SetCellValue("Org Group");
                    ...
                    foreach (var item in Result)
                    {
                        ...
                        row.CreateCell(5).SetCellValue(item.DueDate);
                        row.CreateCell(6).SetCellValue(item.CurrentStatus);
                        row.CreateCell(7).SetCellValue(item.EmployeeName);
                        row.CreateCell(8).SetCellValue(item.ApplicantName);
                        row.CreateCell(9).SetCellValue(item.OrgGroupName);
                        row.CreateCell(10).SetCellValue(item.CurrentTimestamp);
                        row.CreateCell(11).SetCellValue(item.Remarks);

                        row.Cells[0].CellStyle = alignCenter;
                        row.Cells[5].CellStyle = alignCenter;
                        row.Cells[10].CellStyle = alignCenter;

                        row.Cells[2].CellStyle = wrapText;
                        row.Cells[3].CellStyle = wrapText;
                        row.Cells[11].CellStyle = wrapText;
                ...
                await new Common_AuditLog(_iconfiguration, _globalCurrentUser, _env)
                    .AddAuditLog(new Security.Transfer.AuditLog.Form
                    {
                        ...
                        Remarks = "Assigned Tasks Exported",
                        IsSuccess = true,
