        ...
        public async Task OnGet(int ID, int ApplicantID, int EmployeeID)
        {
            if (_globalCurrentUser != null)
            {
                ...
                var status = (await new Common_Maintenance(_iconfiguration, _globalCurrentUser, _env)
                    ...
                    {
                        WorkflowCode = "TASK",
                        CurrentStepCode = ApplicantID > 0 ? ApplicantLogActivityForm.CurrentStatus : EmployeeLogActivityForm.CurrentStatus,
                ...
                status.OrderByDescending(x => x.Order).Select(x => new SelectListItem
                {
                    Value = x.Code,
                    Text = x.Description,
                    Selected = x.Code.Equals(ApplicantID > 0 ? ApplicantLogActivityForm.CurrentStatus : EmployeeLogActivityForm.CurrentStatus)
                }).ToList();
                ...
                var description = await new Common_Maintenance(_iconfiguration, _globalCurrentUser, _env)
                    ...
                    {
                        WorkflowCode = "TASK",
                        Code = ApplicantID > 0 ? ApplicantLogActivityForm.CurrentStatus : EmployeeLogActivityForm.CurrentStatus
                    });
            ...
            if (IsSuccess)
            {
                ...
                await new Common_AuditLog(_iconfiguration, _globalCurrentUser, _env)
                    .AddAuditLog(new Security.Transfer.AuditLog.Form
                    {
                        ...
                        TableID = LogActivityAttachmentForm.ApplicantLogActivityID,
                        Remarks = string.Concat(LogActivityAttachmentForm.ApplicantLogActivityID, " Attachment added"),
                        IsSuccess = true,
                        CreatedBy = _globalCurrentUser.UserID
                    });
            }
