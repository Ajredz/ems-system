...
namespace EMS.FrontEnd.Pages.Workflow.Maintenance
{
    public class IndexModel : SharedClasses.Utilities
    {
        ...
        public void OnGet()
        {
            if (_systemURL != null)
            {
                ViewData["HasAddFeature"] = _systemURL.Where(x => x.URL.ToUpper().Equals("/WORKFLOW/MAINTENANCE/ADD")).Count() > 0 ? "true" : "false";
            }
        }
        ...
        public async Task<(List<GetListOutput>, bool, string)> GetExportData([FromQuery] GetListInput param, bool IsExport)
        {
            var URL = string.Concat(_workflowBaseURL,
                  _iconfiguration.GetSection("WorkflowService_API_URL").GetSection("Maintenance").GetSection("List").Value, "?",
        ...
        public async Task<IActionResult> OnGetDownloadExportList([FromQuery] GetListInput param)
        {
            ...
            if (IsSuccess)
            {
                ...
                string sFileName = "Workflow.xlsx";
                var memory = new MemoryStream();
                using (var fs = new FileStream(Path.Combine(sWebRootFolder, sFileName), FileMode.Create, FileAccess.Write))
                {
                    IWorkbook workbook;
                    workbook = new XSSFWorkbook();
                    ISheet excelSheet = workbook.CreateSheet("Workflow");
                    int irow = 0;
