...
namespace EMS.FrontEnd.Pages.Recruitment.Workflow
{
    public class IndexModel : SharedClasses.Utilities
    {
        ...
        public void OnGet()
        {
            if (_systemURL != null)
            {
                ViewData["HasAddFeature"] = _systemURL.Where(x => x.URL.ToUpper().Equals("/RECRUITMENT/WORKFLOW/ADD")).Count() > 0 ? "true" : "false";
            }
        }
        ...
        public async Task<IActionResult> OnGetListAsync([FromQuery] GetListInput param)
        {
            var URL = string.Concat(_recruitmentBaseURL,
                  _iconfiguration.GetSection("RecruitmentService_API_URL").GetSection("Workflow").GetSection("List").Value, "?",
