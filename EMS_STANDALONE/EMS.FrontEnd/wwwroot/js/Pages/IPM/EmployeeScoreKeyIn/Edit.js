var objEmployeeScoreKeyInEditJS;
...
$(document).ready(function () {
    objEmployeeScoreKeyInEditJS = {
        ...
        Initialize: function () {
            $("#divEmployeeScoreKeyInBodyModal .modal-header").mousedown(handle_mousedown);
            var s = this;
            s.ElementBinding();
            $("#divEmployeeScoreKeyInModal .form-control").not("#ddlPosition").attr("readonly", false);
            $("#divEmployeeScoreKeyInModal #btnEdit").hide();
            $("#divEmployeeScoreKeyInModal #btnSave, #divEmployeeScoreKeyInModal #btnBack").show();
            var param = {
                ID: $("#hdnID").val()
            };
            s.LoadEmployeeScoreJQGrid(param);

            objEmployeeScoreKeyInEditJS.LoadEmployeeScoreStatusHistoryJQGrid({
                ID: $("#hdnID").val(),
            });

            $('#lblRunID').text(objEMSCommonJS.JQGridIDFormat($('#lblRunID').text()));

            objEMSCommonJS.ChangeTab($(".tablinks:nth-child(1)"), 'tabScore', '#divEmployeeScoreKeyInModal');

            if ($('#ddlStatus option[value="FOR_APPROVAL"]').length == 0) {
                $('#ddlStatus option[value="WIP"]').text("FOR_APPROVAL");
                $('#ddlStatus option[value="WIP"]').val("FOR_APPROVAL");
            }
            else {
                $('#ddlStatus option[value="WIP"]').remove();
            }
            $('#ddlStatus option[value="NEW"]').remove();
            $('#ddlStatus option[value="FOR_REVISION"]').remove();
            $('#ddlStatus option[value="APPROVED"]').remove();
        },
        ...
        ElementBinding: function () {
            ...
            $("#divEmployeeScoreKeyInModal #btnSave").click(function () {
                //if (objEMSCommonJS.ValidateBlankFields("#frmEmployeeScore", "#divEmployeeScoreErrorMessage", objEmployeeScoreKeyInEditJS.ValidateTotalScoreFields)) {
                //    ModalConfirmation(MODAL_HEADER, MSG_CONFIRM,
                //        "objEMSCommonJS.PostAjax(true \
                //        , EmployeeScoreKeyInEditPostURL \
                //        , objEmployeeScoreKeyInEditJS.GetFormData() \
                //        , '#divEmployeeScoreErrorMessage' \
                //        , '#divEmployeeScoreKeyInModal #btnSave' \
                //        , objEmployeeScoreKeyInEditJS.EditSuccessFunction);",
                //        "function");

                if (objEMSCommonJS.ValidateBlankFields("#frmEmployeeScore", "#divEmployeeScoreErrorMessage", objEmployeeScoreKeyInEditJS.ValidateTotalScoreFields)) {
                    ModalConfirmation(MODAL_HEADER, MSG_CONFIRM,
                        "objEmployeeScoreKeyInEditJS.SaveFormFunction()",
                        "function");
                }
            });

            $("#btnBack").click(function () {
                LoadPartial(EmployeeScoreKeyInViewURL +
                    "?ID=" + objEmployeeScoreKeyInEditJS.ID +
                    "&OrgGroup=" + objEmployeeScoreKeyInEditJS.OrgGroup +
                    "&Position=" + objEmployeeScoreKeyInEditJS.Position +
                    "&DateFrom=" + objEmployeeScoreKeyInEditJS.TDateFrom +
                    "&DateTo=" + objEmployeeScoreKeyInEditJS.TDateTo, "divEmployeeScoreKeyInBodyModal");
            });

            $("#btnEmployeeExport").click(function () {

                ModalConfirmation(MODAL_HEADER, MSG_CONFIRM, "objEmployeeScoreKeyInEditJS.ExportFunction()",
                    "function");
            });

            $("#divEmployeeScoreKeyInBodyModal .modal-body").scroll(function () {
                if ($("#divEmployeeScoreKeyInBodyModal .modal-body").scrollTop() >= 376) {

                    $("#divEmployeeScoreKeyInBodyModal .ui-jqgrid-htable").css('cssText'
                        , 'width: ' + $("#divEmployeeScoreKeyInBodyModal .ui-jqgrid-hbox").width() + 'px !important');

                    $("#divEmployeeScoreKeyInBodyModal .ui-jqgrid-htable").css({
                        "max-width": $("#divEmployeeScoreKeyInBodyModal .ui-jqgrid-hbox").width() + "px",
                        "position": "fixed",
                        "z-index": "999",
                        "top": $("#divEmployeeScoreKeyInModal").position().top > 0 ? "37px" : "67px"
                    });
                }
                else {
                    $("#divEmployeeScoreKeyInBodyModal .ui-jqgrid-htable").css({
        ...
        ExportFunction: function () {
            var parameters = "&ID=" + objEmployeeScoreKeyInEditJS.ID +
                "&OrgGroup=" + objEmployeeScoreKeyInEditJS.OrgGroup +
                "&Position=" + objEmployeeScoreKeyInEditJS.Position +
                "&DateFrom=" + objEmployeeScoreKeyInEditJS.TDateFrom +
                "&DateTo=" + objEmployeeScoreKeyInEditJS.TDateTo
        ...
        LoadEmployeeScoreJQGrid: function (param) {
            ...
            $("#tblIPMEmployeeScoreKPIScoreList").jqGrid({
                ...
                colModel: [
                    ...
                    { width: 20, name: "Weight", index: "Weight", align: "right", sortable: true, formatter: objEmployeeScoreKeyInEditJS.AddWeightField },
                    { width: 20, name: "Target", index: "Target", align: "right", sortable: true, formatter: objEmployeeScoreKeyInEditJS.AddTargetField },
                    { width: 20, name: "Actual", index: "Actual", align: "right", sortable: true, formatter: objEmployeeScoreKeyInEditJS.AddActualField },
                    { width: 20, name: "Rate", index: "Rate", align: "right", sortable: true, formatter: objEmployeeScoreKeyInEditJS.AddRateField },
                    { width: 20, name: "Total", index: "Total", align: "right", sortable: true, formatter: objEmployeeScoreKeyInEditJS.AddTotalField },
                ...
                loadComplete: function (data) {
                        ...
                        if (data.rows.length > 0) {
                            ...
                            for (var i = 0; i < data.rows.length; i++) {
                                ...
                                $("#txtWeight_" + ctr).blur(function () {
                                    ...
                                    if (parseFloat($("#txtWeight_" + ctr).val().noComma()).toFixed(2) > parseFloat("100"))
                                        $("#txtWeight_" + ctr).val("100.00");

                                    objEmployeeScoreKeyInEditJS.UpdateKPI(ctr);
                                });
                    ...
                    objEmployeeScoreKeyInEditJS.ShowHideFilter();
        ...
        AddWeightField: function (cellvalue, options, rowObject) {
            ...
            return "<input type=\"text\" id=\"txtWeight_" + rowObject.ID + "\" value=\"" + parseFloat(fieldValue).toFixed(2) + "\" class=\"form-control text-amount WeightDynamicFields\" maxlength=\"15\" title=\"Weight\" " + (rowObject.IsEditable ? "" : " tabindex=\"-1\" ") + " readonly>";
        },
    ...
    objEmployeeScoreKeyInEditJS.Initialize();
});